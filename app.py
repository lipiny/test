...
class ServerSettings:
    ...
    def wake_word_enabled(self):
        return bool(
            str(self.wakeword_backend or "").strip()
            and str(self.wake_words or "").strip()
        )
...
class SegmentTimelineTracker:
    ...
    def mark_recording_started(
        self, segment_id, actual_preroll_seconds=None, timestamp=None
    ):
        timestamp = time.time() if timestamp is None else float(timestamp)
        configured_preroll = max(
            0.0, float(self.settings.pre_recording_buffer_duration)
        )
        included_preroll = (
    ...
    def submit(self, job: InferenceJob):
        ...
        with self._condition:
            ...
            if job.kind == "final":
                if (
                    len(state["final"])
                    >= self.settings.max_final_queue_depth_per_session
                ):
...
class SharedEngineWorker:
    ...
    def _warmup(self):
        if not self.settings.model_warmup or self.engine is None:
            return
        warmup_path = (
            Path(__file__).resolve().parents[1] / "RealtimeSTT" / "warmup_audio.wav"
        )
        try:
...
class InferenceScheduler:
    ...
    def healthy(self):
        if self.main_worker.load_error is not None:
            return False
        if (
            self.realtime_worker is not None
            and self.realtime_worker.load_error is not None
        ):
            return False
        return True
            ...
            try:
                ...
                for start in range(0, usable, frame_samples):
                    ...
                    if self.vad.is_speech(
                        frame.astype(np.int16).tobytes(), SERVER_SAMPLE_RATE
                    ):
                        speech_frames += 1
                if checked_frames:
                ...
                LOGGER.debug(
                    "webrtcvad failed; falling back to energy VAD", exc_info=True
                )
...
class RealtimeSession:
        ...
        with self.lock:
            ...
            if not self.recording:
                ...
                if not self.service.try_activate_speaker(self.session_id):
                    ...
                    return (
                        False,
                        "Server active speaker limit reached; audio chunk was ignored.",
                    )
            ...
            if self.recording:
                ...
                recording_seconds = self.recording_sample_count / float(
                    SERVER_SAMPLE_RATE
                )
                silence_seconds = (
                    now - self.last_speech_at if self.last_speech_at else 0.0
                )
                if (
                    ...
                    recording_seconds
                    >= self.settings.max_audio_queue_seconds_per_session
                ):
                    final_job = self._finish_recording_locked("max_duration")
                    if final_job is not None:
                        jobs.append(final_job)
                    warnings.append(
                        "Maximum per-session audio buffer reached; finalized the current segment."
                    )
    ...
    def snapshot(self):
        with self.lock:
            return {
                ...
                "recordingSeconds": self.recording_sample_count
                / float(SERVER_SAMPLE_RATE),
    ...
    def _start_recording_locked(self, now):
        ...
        self.recording_sample_count = sum(
            int(frame.size) for frame in self.recording_frames
        )
        self.timeline.mark_recording_started(
            self.active_segment_id,
            actual_preroll_seconds=self.prebuffer_sample_count
            / float(SERVER_SAMPLE_RATE),
            timestamp=time.time(),
        )
        self.prebuffer.clear()
    ...
    def _finish_recording_locked(self, reason):
        ...
        recording_seconds = (
            audio.size / float(SERVER_SAMPLE_RATE) if audio is not None else 0.0
        )
    ...
    def _maybe_create_realtime_job_locked(self, now):
        ...
        if pause > 0 and now - self.last_realtime_submit_at < pause:
            return None
        if self.recording_sample_count < int(
            self.settings.realtime_min_audio_seconds * SERVER_SAMPLE_RATE
        ):
            return None
        audio = self._recording_audio_float32_locked(
            max_seconds=self.settings.realtime_max_audio_seconds
        )
        ...
        return InferenceJob(
            ...
            + (self.settings.max_realtime_queue_age_ms / 1000.0),
        )
    ...
    def _append_prebuffer_locked(self, samples):
        ...
        max_samples = int(
            self.settings.pre_recording_buffer_duration * SERVER_SAMPLE_RATE
        )
        while (
            max_samples >= 0
            and self.prebuffer_sample_count > max_samples
            and self.prebuffer
        ):
...
class RecorderBackedRealtimeSession:
    ...
    def stop_streaming(self):
        ...
        try:
            ...
            LOGGER.debug(
                "Could not flush buffered audio for %s", self.session_id, exc_info=True
            )
        finally:
            self.service.deactivate_speaker(self.session_id)
        self.publish_status("idle")
    ...
    def close(self):
        ...
        try:
            ...
            LOGGER.debug(
                "Recorder shutdown failed for %s", self.session_id, exc_info=True
            )
    ...
    def clear(self):
        ...
        try:
            ...
            LOGGER.debug(
                "Recorder abort failed during clear for %s",
                self.session_id,
                exc_info=True,
            )
        self.service.manager.publish_session(
    ...
    def ingest_audio_packet(self, packet):
        ...
        with self.lock:
            if not self.streaming:
                ...
                return (
                    False,
                    "Audio stream is stopped; send a start command before audio packets.",
                )
        try:
    ...
    def _finish_wakeword_followup(self, generation=None):
        with self.lock:
            if (
                ...
                and generation != self._wakeword_followup_generation
            ):
                return False
    ...
    def _clear_recorder_followup_gate_locked(self):
        ...
        try:
            ...
            if self._recorder_wake_word_timeout_before_followup is not None:
                recorder.wake_word_timeout = (
                    self._recorder_wake_word_timeout_before_followup
                )
    ...
    def _enforce_recording_duration(self, samples):
        ...
        max_samples = int(
            self.settings.max_audio_queue_seconds_per_session * SERVER_SAMPLE_RATE
        )
        if max_samples <= 0:
            return None
    ...
    def _force_finalize_after_limit(self):
        ...
        try:
            ...
            LOGGER.debug(
                "Could not force-finalize long recording for %s",
                self.session_id,
                exc_info=True,
            )
        finally:
...
class SessionStore:
    ...
    def add(self, session):
        with self._lock:
            ...
            if (
                not reserved
                and self._session_slots_used_locked() >= self.settings.max_sessions
            ):
                self.rejected_sessions += 1
                return False
            self._reserved_session_ids.discard(session.session_id)
    ...
    def snapshots(self):
        ...
        return {
            ...
            "sessions": {
                session.session_id: session.snapshot() for session in sessions
            },
        }
    ...
    def packet_to_server_samples(self, packet):
        ...
        if isinstance(channels, bool) or not isinstance(channels, int) or channels <= 0:
            raise AudioPacketError(
                "audio packet metadata field 'channels' must be a positive integer"
            )
        if channels > 8:
            raise AudioPacketError(
                "audio packet metadata field 'channels' must be at most 8"
            )
        ...
        if len(packet.audio) % frame_width:
            raise AudioPacketError(
                "pcm_s16le audio packet is not aligned to whole frames"
            )
        ...
        if channels > 1:
            ...
            samples = (
                samples[:usable].reshape(-1, channels).mean(axis=1).astype(np.int16)
            )
    ...
    def update_settings(self, updates):
        ...
        for name, value in updates.items():
            if name in STARTUP_ONLY_SETTINGS:
                ...
                continue
            if (
                name not in ACTIVE_RUNTIME_SETTINGS
                and name not in NEW_SESSION_RUNTIME_SETTINGS
            ):
                rejected[name] = {
                    "reason": "unknown",
...
def read_wav_float32(path: Path):
    ...
    return AudioData(
        samples=samples.astype(np.float32) / INT16_MAX_ABS_VALUE,
...
def create_app(
    ...
    async def send_transcript(request: Request):
        ...
        try:
            ...
            return JSONResponse(
                {
                    "ok": False,
                    "message": f"Receiver returned HTTP {exc.code}",
                    "receiverBody": error_body,
                },
                status_code=502,
            )
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="RealtimeSTT FastAPI browser streaming server"
    )
    ...
    parser.add_argument(
        ...
        dest="ssl_certfile",
        default=None,
        help="Path to the SSL certificate file (PEM) for HTTPS",
    )
    parser.add_argument(
        "--ssl-keyfile",
        dest="ssl_keyfile",
        default=None,
        help="Path to the SSL key file (PEM) for HTTPS",
    )
    parser.add_argument(
        "--profile",
        "--tuning-profile",
    ...
    parser.add_argument(
        "--realtime-engine-options", dest="realtime_transcription_engine_options"
    )
    ...
    parser.add_argument(
        "--realtime-callback", choices=("update", "stabilized"), default="update"
    )
    ...
    parser.add_argument(
        "--realtime-boundary-detector-sensitivity", type=float, default=0.6
    )
    ...
    parser.add_argument(
        "--max-audio-queue-seconds-per-session", type=float, default=30.0
    )
...
def parse_float_tuple(value, flag_name):
    ...
    try:
        ...
        raise SystemExit(
            f"{flag_name} must be a comma-separated list of numbers"
        ) from exc
...
def settings_from_args(args):
    ...
    return ServerSettings(
        ...
        realtime_transcription_engine=normalize_engine_name(
            args.realtime_transcription_engine
        ),
        transcription_engine_options=parse_json_object(
            args.transcription_engine_options, "--engine-options"
        ),
        ...
        min_length_of_recording=_value_or_default(
            args, defaults, "min_length_of_recording"
        ),
        min_gap_between_recordings=args.min_gap_between_recordings,
        post_speech_silence_duration=_value_or_default(
            args, defaults, "post_speech_silence_duration"
        ),
        silero_sensitivity=args.silero_sensitivity,
        webrtc_sensitivity=args.webrtc_sensitivity,
        realtime_processing_pause=_value_or_default(
            args, defaults, "realtime_processing_pause"
        ),
        ...
        early_transcription_on_silence=_value_or_default(
            args, defaults, "early_transcription_on_silence"
        ),
        initial_prompt=args.initial_prompt,
        initial_prompt_realtime=args.initial_prompt_realtime,
        wakeword_backend=(
            args.wakeword_backend or ("pvporcupine" if args.wake_words else "")
        ),
